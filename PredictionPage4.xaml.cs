...
public partial class PredictionPage4 : ContentPage
{
   
    private FileResult selectedPhoto; // Field to store the selected photo
    private static FileResult _photoGauche;
    private static FileResult _photoDroite;
    private static FileResult _photoAvant;
    private static FileResult _selectedPhoto;
    private readonly InferenceSession _resnetSession;
    private readonly InferenceSession _vitSession;
    public PredictionPage4(FileResult photoGauche, FileResult photoDroite, FileResult photoAvant)
    {
        InitializeComponent();
        ResultImage.Source = "cart.jpeg";

        _photoGauche = photoGauche;
        _photoDroite = photoDroite;
        _photoAvant = photoAvant;
    ...
    private async void ProcessAndCheck(object sender, EventArgs e)
    {
        ...
        if (file != null)
        {
            ...
            if (directionPrediction == "arrier" && completePrediction == "1") // Assumes '1' means complete
            {
                PredictionLabel.Text = "backward direction , complete side.";
                NextButton.IsEnabled = true;
                RefreshButton.IsEnabled = true;
            }
            else if (directionPrediction == "arrier")
            {
                PredictionLabel.Text = "backward direction , but not complete";
            }
            else
            {
                PredictionLabel.Text = "Direction is not backward";
            }
    ...
    {
        await Navigation.PushAsync(new PageEnd(_photoGauche, _photoDroite, _photoAvant, _selectedPhoto));
    }
