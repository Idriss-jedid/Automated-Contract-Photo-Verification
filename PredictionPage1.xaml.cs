...
namespace MauiApp3
{
    public partial class PredictionPage1 : ContentPage
    {
        private FileResult _selectedPhoto; // Field to store the selected photo

        // Load ONNX models once
        private readonly InferenceSession _resnetSession;
        private readonly InferenceSession _vitSession;

        public PredictionPage1()
        {
        ...
        private async void ProcessAndCheck(object sender, EventArgs e)
        {
            ...
            if (file != null)
            {
                ...
                if (directionPrediction == "gauche" && completePrediction == "1") //  '1' means complete
                {
                    PredictionLabel.Text = "Left direction, complete side.";
                    NextButton.IsEnabled = true;
                    RefreshButton.IsEnabled = true;
                }
                else if (directionPrediction == "gauche")
                {
                    PredictionLabel.Text = "Left direction, but not complete";
                }
                else
                {
                    PredictionLabel.Text = "Direction is not left";
                }
            }
        }
        ...
        {
            await Navigation.PushAsync(new PredictionPage2(_selectedPhoto));
        }
