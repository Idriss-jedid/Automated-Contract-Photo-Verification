...
public partial class PredictionPage3 : ContentPage
{
    ...
    private async void ProcessAndCheck(object sender, EventArgs e)
    {
        ...
        if (file != null)
        {
            ...
            if (directionPrediction == "avant" && completePrediction == "1") // Assumes '1' means complete
            {
                PredictionLabel.Text = "Forward direction, complete side.";
                NextButton.IsEnabled = true;
                RefreshButton.IsEnabled = true;
            }
            else if (directionPrediction == "avant")
            {
                PredictionLabel.Text = "Forward direction, but not complete";
            }
            else
            {
                PredictionLabel.Text = "Direction is not forward";
            }
    ...
    {
        await Navigation.PushAsync(new PredictionPage4(_selectedPhoto,_photoGauche,_photoDroite));
    }
